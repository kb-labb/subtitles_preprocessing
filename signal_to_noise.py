...
def main():
    ...
    for p in program_names:
        wav = os.path.join(args.data, p, 'file.wav')
        programs.append(wav)
    df = pd.DataFrame(programs, columns=['audio']) 

    df["snr"]= -1
    df['snr'] = df.swifter.apply(lambda x: wada_snr(x['audio']), axis=1)
    print('df',df )
