...
def make_splits(metadata_dir, output_dir, ratios):
    ...
    metadata = pd.read_csv(
        os.path.join(metadata_dir, "metadata_full.csv")
    )  # file with chunks' filenames and transcriptions
    train_size, test_size, val_size = ratios[0], ratios[1], ratios[2]

    metadata["programid"] = metadata["file_name"].apply(
        lambda x: ("_").join(re.findall(r"[\w']+", x)[:-2])
    )
