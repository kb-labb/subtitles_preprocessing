...
def main() -> None:
    ...
    for p in program_names:
        ...
        df["audio"] = df["files"].map(
            lambda x: [file for file in x if file.endswith(".wav")][0]
        )
        df["srt"] = df["files"].map(
            lambda x: [file for file in x if file.endswith(".srt")][0]
        )
        df["program"] = p
        ...
        df_subs = []
        for x, srt, audio in tqdm(
            zip(df["program"].tolist(), df["srt"].tolist(), df["audio"].tolist()),
            total=df.shape[0],
        ):
        ...
        df_subs["force_start"] = df_subs["start_ms"][
            abs(df_subs["start_ms"].diff()) > 4000
        ]
        ...
        df_groups["end_relative"] = (
            (np.round(df_groups["end_relative"] / 20)) * 20 / 1000
        )
        ...
        df_groups["text_timestamps"] = (
            df_groups["start_timestamp"]
            + df_groups["text"]
            + df_groups["end_timestamp"]
        )
        ...
        df_groups[
            ...
            os.path.join(args.output, p, "subs_preprocessed.parquet"), index=False
        )
        logging.info("Ending")
